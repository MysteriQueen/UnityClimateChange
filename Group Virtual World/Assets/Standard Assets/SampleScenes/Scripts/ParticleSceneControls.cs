...
namespace UnitySampleAssets.SceneUtils {
    public class ParticleSceneControls : MonoBehaviour {
        ...
        private float particleMultiplier = float.MaxValue;
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    if ((pos - lastPos).magnitude > selected.minDist)
                    {
                        ...
                        if (selected.mode != Mode.Trail || instance == null)
                        {
                            ...
                            if (particleMultiplier == float.MaxValue) {
                                ParticleSystem.MainModule main = instance.GetComponent<ParticleSystem>().main;
                                main.startLifetimeMultiplier = multiply;
                            }
        ...
        private void Select(int i)
        {
            ...
            particleMultiplier = selected.transform.GetComponent<ParticleSystem>().main.startLifetimeMultiplier;
            multiply = 1;
            if (clearOnChange)
            {
