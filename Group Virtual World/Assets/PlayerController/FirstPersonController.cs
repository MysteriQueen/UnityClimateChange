using UnityEngine;
using UnityEngine.UI; // For crosshair image
...
public class FirstPersonController : MonoBehaviour {
    ...
    public float crouchStepTime = 10f;
    public float crouchSpeedStepTime = 10f;

    // Internal Variables
    private const int CROUCHED = 0;
    private const int CROUCHING = 1;
    private const int UNCROUCHING = 2;
    private const int STANDING = 3;
    private int crouchStatus = STANDING;
    ...
    public int exclusionLayer = 0;

    //private InteractionAnimationController raycastedObj; // Replace with generic representation in group project

    public KeyCode[] interactInputs = {KeyCode.Mouse0, KeyCode.E, KeyCode.F};
    ...
    private void Update() {
        ...
        if (!stateManager.Paused) {
            ...
            if (cameraCanMove) {
                yaw = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * mouseSensitivity * stateManager.Sensitivity;

                if (!invertCamera) {
                    pitch -= Input.GetAxis("Mouse Y") * mouseSensitivity * stateManager.Sensitivity;
                } else {
                    // Inverted Y
                    pitch += Input.GetAxis("Mouse Y") * mouseSensitivity * stateManager.Sensitivity;
                }
            ...
            // Create mask which excludes the specified exlusion layer, but includes the layer we want to interact with.
            int mask = 1 << layerMaskInteract.value;
            ...
            /*if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, rayLength, mask)) {
                if (hit.collider.CompareTag(interactibleTag)) {
                    raycastedObj = hit.collider.gameObject.GetComponent<InteractionAnimationController>();

                    foreach(KeyCode key in interactInputs) {
            ...
            if (enableCrouch) {
                if (crouchStatus == CROUCHING || Input.GetKey(crouchKey))
                    Crouch();

                bool crouchKeyUp = Input.GetKeyUp(crouchKey);
                if (crouchStatus == UNCROUCHING || (crouchKeyUp && holdToCrouch) || (crouchKeyUp && crouchStatus != CROUCHING)) {
                    UnCrouch();
                }
            }
    ...
    void FixedUpdate() {
        ...
        if (playerCanMove) { // ISSUE
            // Calculate how fast we should be moving
            Vector3 targetVelocity = new Vector3(0.0f, 0.0f, 0.0f);
            if (!stateManager.Paused)
            ...
            } else { // All movement calculations while walking
                isSprinting = false;
    ...
    private void Jump() {
        ...
        // When crouched and using toggle system, will uncrouch for a jump (only if the player is not mid-crouch (isMidCrouch))
        if(!holdToCrouch) {
            UnCrouch(true);
        }
    }
     ...
     */
    private void UnCrouch(bool bypassLerp = false) {
        if (!bypassLerp) {
            transform.localScale = new Vector3(originalScale.x, Mathf.Lerp(transform.localScale.y, originalScale.y, crouchStepTime * Time.deltaTime), originalScale.z);
            walkSpeed = Mathf.Lerp(walkSpeed, originalWalkSpeed, crouchSpeedStepTime * Time.deltaTime);

            crouchStatus = UNCROUCHING;

            if (originalScale.y - transform.localScale.y < 0.005) {
     ...
     */
    private void Crouch() {
        transform.localScale = new Vector3(originalScale.x, Mathf.Lerp(transform.localScale.y, crouchHeight, crouchStepTime * Time.deltaTime), originalScale.z);
        walkSpeed = Mathf.Lerp(walkSpeed, originalWalkSpeed * speedReduction, crouchSpeedStepTime * Time.deltaTime);

        crouchStatus = CROUCHING;

        if (transform.localScale.y - crouchHeight < 0.005) {
    ...
    private void HeadBob() {
        if (isWalking) {
            ...
            } else if (crouchStatus <= CROUCHING ) { // < CROUCHING = Crouching or Crouched (1, 0)
...
public class FirstPersonControllerEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        GUILayout.Label("Originally by Jess Case (Modular FIrst Person Controller)", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();
        ...
        fpc.jumpPower = EditorGUILayout.Slider(new GUIContent("Jump Power", "Determines how high the player will jump."), fpc.jumpPower, .1f, 500f);
        GUI.enabled = true;
        ...
        fpc.crouchStepTime = EditorGUILayout.Slider(new GUIContent("Crouch Step Time", "Determines how long it takes to crouch"), fpc.crouchStepTime, 1f, 50f);
        fpc.crouchSpeedStepTime = EditorGUILayout.Slider(new GUIContent("Crouch Step Time", "Determines how quickly the player's speed is decreased when crouching"), fpc.crouchSpeedStepTime, 1f, 50f);
        GUI.enabled = true;
        ...
        fpc.rayLength = EditorGUILayout.IntField(new GUIContent("Ray Length", "The length of the ray to cast for interacting with the world"), fpc.rayLength);
        fpc.layerMaskInteract = (LayerMask)EditorGUILayout.LayerField(new GUIContent("Interative Layer", "The layer which allows for interaction wioth"), fpc.layerMaskInteract);
        fpc.exclusionLayer = EditorGUILayout.LayerField(new GUIContent("Exclusion Layer", "Exclude this layer from being interactable"), fpc.exclusionLayer);
